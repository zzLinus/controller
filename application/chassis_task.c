...
static void chassis_init(chassis_move_t *chassis_move_init)
{
    ...
    const static fp32 chassis_yaw_pid[3] = {CHASSIS_FOLLOW_GIMBAL_PID_KP, CHASSIS_FOLLOW_GIMBAL_PID_KI, CHASSIS_FOLLOW_GIMBAL_PID_KD};
		...
		const static fp32 chassis_no_follow_yaw_pid[3] = {4.5, 0.0002, 0};
		#else
			#ifdef INFANTRY2_ZHANG
			const static fp32 chassis_no_follow_yaw_pid[3] = {3.5, 0.0002, 0};
			#endif
		#endif
    ...
    PID_init(&chassis_move_init->chassis_angle_pid, PID_POSITION, chassis_yaw_pid, CHASSIS_FOLLOW_GIMBAL_PID_MAX_OUT, CHASSIS_FOLLOW_GIMBAL_PID_MAX_IOUT);
		...
		PID_init(&chassis_move_init->chassis_no_follow_angle_pid, PID_POSITION, chassis_no_follow_yaw_pid, CHASSIS_FOLLOW_GIMBAL_PID_MAX_OUT, CHASSIS_FOLLOW_GIMBAL_PID_MAX_IOUT);
...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
    ...
    if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW)
    {
        ...
        //设置控制相对云台角度
        chassis_move_control->chassis_relative_angle_set = rad_format(angle_set);  //此模式下如果不开启摇摆，angle_set始终为0
...
{
		if(gimbal_yaw_direction == GIMBAL_YAW_FORWARD)
		{
		...
		else if(gimbal_yaw_direction == GIMBAL_YAW_BACKWARD)
		{
			wheel_speed[2] = -vx_set - vy_set + (CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set;
			wheel_speed[3] = vx_set - vy_set + (CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set;
			wheel_speed[0] = vx_set + vy_set + (-CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set;
...
static void chassis_control_loop(chassis_move_t *chassis_move_control_loop)
{
    ...
    for (i = 0; i < 4; i++)
    {
		...
		fp32 max_whell_speed = (chassis_behaviour_mode == CHASSIS_SPIN) ? MAX_WHEEL_SPIN_SPEED : MAX_WHEEL_SPEED;
