...
void UI_Send(void){
    HAL_UART_Transmit(&huart6, UI_com, UI_tot, 0xffff);
    UI_tot = 0;
}
...
void String_Draw(String_Data *image,char imagename[3],u32 Graph_Operate,u32 Graph_Layer,u32 Graph_Color,u32 Graph_Size,u32 Graph_Digit,u32 Graph_Width,u32 Start_x,u32 Start_y,char *String_Data)
{
    int i;
...
void UI_open(UI_control_t *UI) {
    UI->UI_mode = 1;
    String_Draw(&UI->fric_name, "frs", UI_Graph_ADD, 9, UI_Color_White, 10, 6, 3, FRIC_NAME_START_X, FRIC_NAME_START_Y, "FRIC");
    String_Draw(&UI->fric_name, "sps", UI_Graph_ADD, 9, UI_Color_White, 10, 6, 3, SPIN_NAME_START_X, SPIN_NAME_START_Y, "SPIN");
    Line_Draw(&UI->fric_graph, "frg", UI_Graph_ADD, 9, UI->fric_open ? UI_Color_Green : UI_Color_Main, 5, FRIC_NAME_START_X, FRIC_NAME_START_Y, FRIC_NAME_START_X + 20, FRIC_NAME_START_Y);
    Line_Draw(&UI->spin_graph, "sps", UI_Graph_ADD, 9, UI->spin_open ? UI_Color_Green : UI_Color_Main, 5, SPIN_NAME_START_X, SPIN_NAME_START_Y, SPIN_NAME_START_X + 20, SPIN_NAME_START_Y);
    Circle_Draw(&UI->accuracy_graph, "acc", UI_Graph_ADD, 9, UI_Color_White, 10, 700, 500, 20);
    UI_ReFresh(2, UI->fric_graph, UI->spin_graph);
    UI_ReFresh(1, UI->accuracy_graph);
    String_ReFresh(UI->fric_name);
    String_ReFresh(UI->spin_name);
}

void UI_close(UI_control_t *UI){
    UI->UI_mode = 0;
    UI_Delete(UI_Data_Del_ALL, 9);
}

void UI_set_fric(uint8_t mode){
    UI_control.fric_open = mode;
}

void UI_set_spin(uint8_t mode) {
    UI_control.spin_mode = mode;
}
...
void UI_set_control(UI_control_t *UI){
    ...
    if(UI_control.fric_mode != UI_control.fric_open) {
        Line_Draw(&UI_control.fric_graph, "frg", UI_Graph_ADD, 9, UI_control.fric_open ? UI_Color_Green : UI_Color_Main, 5,
                  FRIC_NAME_START_X, FRIC_NAME_START_Y, FRIC_NAME_START_X + 20, FRIC_NAME_START_Y);
        UI_control.fric_mode = UI_control.fric_open;
    }
    if(UI_control.spin_mode != UI_control.spin_open){
        Line_Draw(&UI_control.spin_graph, "sps", UI_Graph_ADD, 9, UI_control.spin_open ? UI_Color_Green : UI_Color_Main, 5,
                  SPIN_NAME_START_X, SPIN_NAME_START_Y, SPIN_NAME_START_X + 20, SPIN_NAME_START_Y);
